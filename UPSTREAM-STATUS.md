...
|---|---|---|
| `UPSTREAM-575-redeclaration-anchor-races-under-threading.md` | nondeterministic diagnostic output | **SYMPTOM REPRODUCED**, 2026-08-07. `redeclaration_error` (`checker.cpp:2027`) anchors on `prev->token` — the entity *already in the scope* — so the error anchor and the `at` continuation swap depending on which file's declaration was collected first. Measured on `core/rexcode/isa/mos65816/tools`: **2/30 flips threaded, 0/20 with `-thread-count:1`**, which is the control placing the cause in concurrent collection. Second control: the line-for-line port, which orders its scope insertions, is 30/30 stable on the same input threaded. Mechanism (racing collection) is **inferred from the emitter's source, not instrumented**. **Filed as PR #7253**, which fixes n = 2 by ordering the pair positionally. **n > 2 remains open**: with three declarations the *set* of reported pairs varies (39/40 vs 1/40 on a three-file probe), because the incumbent appears in every pair — sorting a pair cannot fix a set, and the real fix is deterministic collection order. One outlier in that probe names two different incumbents in a single run, which the exclusive lock in `scope_insert_with_name` should prevent; recorded as an open question, not guessed at. Same family as LEDGER #197/#341, but with a named site and a clean control. |
...
| `UPSTREAM-582-constant-string-slice-assertions.md` | **crash** x2, deterministic | **BOTH REPRODUCED 5/5**, re-verified from the recorded inputs on 2026-08-08 against `dev-2026-08:a64cb7bfd`. **A:** `check_slice_expr` diagnoses `low > high` at `check_expr.cpp:12208` but does not `return`, so `substring()` is called with lo>hi and `string.cpp:77 GB_ASSERT_MSG(lo <= hi && hi <= max)` fires. Input: `S :: "hello"; x := S[3:1]`. Affects both the String and String16 folding branches (12263, 12270). **B:** `check_expr.cpp:12081 GB_ASSERT(o->value.kind == ExactValue_String16)` fires on ANY slice of a `string16` constant, valid indices included, because such a constant keeps the literal's `ExactValue_String` -- it is never re-expressed in UTF-16 on conversion. Input: `S: string16 : "hello"; x := S[0:2]`. CONTROL: the same declaration unsliced exits 0, placing the trigger in the slice. Both cited lines re-read in `src/` at filing time. The port reproduces neither -- A is a documented divergence with a demonstrated reason, B degrades instead of aborting (see the port-side tail, LEDGER #583). NOT YET FILED. |
