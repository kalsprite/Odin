...
builtin_proc_infos := [Builtin_Proc_Id]Builtin_Proc_Info {
	...
	.Atomic_Load = {name = "atomic_load", arg_count = 1, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Load_Explicit = {name = "atomic_load_explicit", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Add = {name = "atomic_add", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Add_Explicit = {name = "atomic_add_explicit", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Sub = {name = "atomic_sub", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Sub_Explicit = {name = "atomic_sub_explicit", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_And = {name = "atomic_and", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_And_Explicit = {name = "atomic_and_explicit", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Nand = {name = "atomic_nand", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Nand_Explicit = {name = "atomic_nand_explicit", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Or = {name = "atomic_or", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Or_Explicit = {name = "atomic_or_explicit", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Xor = {name = "atomic_xor", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Xor_Explicit = {name = "atomic_xor_explicit", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Exchange = {name = "atomic_exchange", arg_count = 2, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Exchange_Explicit = {name = "atomic_exchange_explicit", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Compare_Exchange_Strong = {name = "atomic_compare_exchange_strong", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Compare_Exchange_Strong_Explicit = {name = "atomic_compare_exchange_strong_explicit", arg_count = 5, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Compare_Exchange_Weak = {name = "atomic_compare_exchange_weak", arg_count = 3, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Atomic_Compare_Exchange_Weak_Explicit = {name = "atomic_compare_exchange_weak_explicit", arg_count = 5, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	// Objective-C runtime builtins
	// C++ Reference: checker_builtin_procs.hpp:350-356
	.Objc_Send = {name = "objc_send", arg_count = 3, variadic = true, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	...
	.Objc_Register_Selector = {name = "objc_register_selector", arg_count = 1, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Objc_Register_Class = {name = "objc_register_class", arg_count = 1, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Objc_Ivar_Get = {name = "objc_ivar_get", arg_count = 1, variadic = false, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	...
	.Syscall = {name = "syscall", arg_count = 1, variadic = true, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
	.Syscall_Bsd = {name = "syscall_bsd", arg_count = 1, variadic = true, kind = .Expr, pkg = .Intrinsics, ignore_results = true},
...
Checker :: struct {
	...
	// ---- CHECKER-OWNED RUNTIME TYPES (#566) -------------------------------------------------
	// These 89 were package globals. They are resolved out of, or allocated into, THIS checker's
	// scopes, so they cannot outlive it -- which is why reset_runtime_type_globals had to exist at
	// all (LEDGER #368: "one checker's types escaped into the next check").
	//
	// Moving them here removes the MECHANISM, not merely the storage. init_core_type_info's
	// once-guard was `if t_type_info != nil` on a GLOBAL, so it was process-wide: a second checker
	// that adopted the runtime session hit the guard and silently reused the first checker's types.
	// The guard is now per-checker, so the second checker re-resolves through the SAME shared scope
	// and gets the same ^Type objects -- shared by construction instead of by cache.
	//
	// The other 73 `t_*` globals stay package-level in types.odin. They are built by
	// init_basic_types from build_context alone and legitimately outlive any Checker. The split is
	// legible at every use site (`c.t_x` vs bare `t_x`) and ENFORCED by .claude/tools/splitcheck.py.
	//
	// PRECONDITION THIS DOES NOT REMOVE: build_context is still a package-level global
	// (build_settings.odin), and init_basic_types is still reached through a process-wide
	// `t_int == nil` guard. So two checkers in one process may not target different platforms --
	// the first one's target freezes the basics for the process. #566 makes the checker re-entrant
	// across SESSIONS, not across TARGETS. LEDGER #572, mir's condition 2.
