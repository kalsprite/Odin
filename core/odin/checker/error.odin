...
// C++ Reference: error.cpp any_errors
...
// C++ Reference: error.cpp any_warnings
...
// C++ Reference: error.cpp push_error_value
...
// C++ Reference: error.cpp pop_error_value
...
// C++ Reference: error.cpp try_pop_error_value
...
// C++ Reference: error.cpp get_error_value
...
// C++ Reference: error.cpp terminal_set_colours
...
// C++ Reference: error.cpp terminal_reset_colours
...
// C++ Reference: error.cpp error_out (error_out) and error.cpp default_error_out_va (default_error_out_va)
...
// C++ Reference: error.cpp error_out_empty
error_out_empty :: proc() {
	error_out("")
}

// error_out_pos outputs a formatted position
// C++ Reference: error.cpp error_out_pos
...
// C++ Reference: error.cpp error_out_coloured
...
show_error_on_line :: proc(pos: tokenizer.Pos, end: tokenizer.Pos) -> int {
	...
	// Line display constants (C++ Reference: error.cpp show_error_on_line)
	MAX_LINE_LENGTH :: 80
	ELLIPSIS_PADDING :: 8   // `...  ...`
	MIN_LEFT_VIEW :: 8
	...
	// Line truncation with windowing (C++ Reference: error.cpp show_error_on_line)
...
// C++ Reference: error.cpp error_va
error_va :: proc(pos: tokenizer.Pos, end: tokenizer.Pos, format: string, args: ..any) {
	...
	// C++ error.cpp error_va does `print_all_errors(); exit(1);` here. That is correct for a
...
//
// C++ Reference: error.cpp error_no_newline_va. Two things differ from error_va, and both are visible in
// the output:
//
...
// C++ Reference: error.cpp warning_va
...
// C++ Reference: error.cpp error_line_va
...
syntax_error_va :: proc(pos: tokenizer.Pos, end: tokenizer.Pos, format: string, args: ..any) {
	// DELIBERATE DIVERGENCE FROM C++ (CPP_DEVIATIONS.md [EMBED-1]).
	// C++ error.cpp syntax_error_va does `print_all_errors(); exit(1);` here. Same reasoning as
...
// C++ Reference: error.cpp syntax_warning_va
...
//
// C++ Reference: parser_pos.cpp ast_token (`ast_token`). Nearly every arm either returns the
...
//
//   Assign_Stmt  -> the operator      (parser_pos.cpp ast_token).  `y = f()` reports at the `=`.
//   Deref_Expr   -> the trailing `^`  (parser_pos.cpp ast_token).  Odin's deref is postfix.
//
...
ast_token_pos :: proc(node: ^ast.Node) -> tokenizer.Pos {
	...
	case ^ast.Implicit_Selector_Expr:
		// C++ parser_pos.cpp ast_token: reports at the SELECTOR, not the leading '.'.
		if n.field != nil {
			return n.field.pos
		}
	case ^ast.Field_Value:
		// C++ parser_pos.cpp ast_token. The point is the RECURSION: C++'s ast_token walks
...
// C++ Reference: error.cpp syntax_warning
syntax_warning :: proc {
...
// C++ Reference: error.cpp compiler_error
//
...
// C++ Reference: error.cpp exit_with_errors
//
...
// C++ Reference: error.cpp error_value_cmp
...
// C++ Reference: error.cpp error_article
...
// C++ Reference: error.cpp print_all_errors
print_all_errors :: proc() {
	...
	// C++ Reference: error.cpp print_all_errors - GB_ASSERT(any_errors() || any_warnings())
	//
...
// C++ Reference: common.cpp levenstein_distance_case_insensitive
...
// C++ Reference: check_expr.cpp check_did_you_mean_print.
//
...
check_did_you_mean_scope :: proc(name: string, scope: ^Scope, prefix := "") {
	...
	// C++ (check_expr.cpp check_did_you_mean_scope) walks scope->elements in raw hash order, then sorts by
